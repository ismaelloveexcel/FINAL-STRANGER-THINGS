    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6">
      ...
      <div className="flex justify-between items-start w-full">
        <div className="flex flex-col gap-3">
          ...
          {/* Level Info */}
          <div className="bg-black/70 border border-cyan-500/30 p-3 rounded backdrop-blur-sm">
            <div className="flex items-center gap-2 mb-2">
              <Zap className="w-5 h-5 text-cyan-400" />
              <span className="font-mono text-cyan-300 text-sm">LEVEL {currentLevel}: {config.name.toUpperCase()}</span>
            </div>
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-mono text-white text-xs">
                TARGETS: {enemiesKilled}/{requiredKills}
              </span>
            </div>

            {/* Progress Bar */}
            <div className="w-56 h-2 bg-black/50 border border-cyan-900 rounded overflow-hidden">
              <div
              ...
              <div className="space-y-6">
                {isGameOver ? (
                  ...
                  <div className="space-y-6 text-center">
                    <div className="text-cyan-200/70 font-mono text-sm space-y-3 border border-cyan-900/30 p-4 bg-black/30 rounded">
                      <p className="text-red-400 font-bold text-lg mb-2">{STORY_TEXT.intro.title}</p>
                      {STORY_TEXT.intro.text.map((line, i) => (
                        <p key={i} className="text-white/80 italic">{line}</p>
                      ))}
                      <div className="border-t border-cyan-900/30 pt-3 mt-3">
                        <p className="text-cyan-300 font-bold">CONTROLS:</p>
                        <p>WASD to Move | SPACE to Jump | MOUSE to Look/Shoot</p>
                      </div>
                      <p className="mt-3 text-amber-300 text-xs">Fight through 3 levels:</p>
